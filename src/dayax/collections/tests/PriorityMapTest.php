...
use dayax\collections\PriorityMap;

class PriorityMapTest_MapItem {
  public $data='data';
}
...
class PriorityMapTest extends TestCase
{
  ...
  public function setUp() {
  	// test that PriorityMap complies with TMap
    $this->map=new PriorityMap;
    $this->item1=new PriorityMapTest_MapItem;
    $this->item2=new PriorityMapTest_MapItem;
    $this->item3=new PriorityMapTest_MapItem;
    $this->item4=new PriorityMapTest_MapItem;
    $this->item5=new PriorityMapTest_MapItem;
  ...
  public function testConstruct() {
    $a=array(1,2,'key3'=>3);
    $map=new PriorityMap($a);
    $this->assertEquals(3,$map->getPriorityCount());
    $this->assertEquals(3,$map->getCount());
    $map2=new PriorityMap($this->map);
    $this->assertEquals(2,$map2->getCount());

	/* Test the priority functionality of PriorityMap  */

    $map3=new PriorityMap($this->map, false, 100, -1);
  ...
  public function testGetKeys() {
	...
	public function testCanNotAddWhenReadOnly() {
		$map = new PriorityMap(array(), true);
		try {
			$map->add('key', 'value');
	...
	public function testCanNotRemoveWhenReadOnly() {
		$map = new PriorityMap(array('key' => 'value'), true);
		try {
			$map->remove('key');
		} catch(\dayax\collections\InvalidOperationException $e) {
			return;
	...
	public function testToArray() {
		$map = new PriorityMap(array('key' => 'value'));
	...
	public function testRemoveWithPriorityAndItemsAtWithPriority() {
        ...
        $this->assertEquals(2,count($this->map->itemsAtPriority()));
	...
	public function testIteratorAndArrayWithPriorities() {
		...
		// This is the primary reason for a PriorityMap
	...
	public function testGetPriorities() {
		...
		//@TODO should return null to invalid index
